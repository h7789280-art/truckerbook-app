...
export default function Overview({ userName, userId, profile, onOpenProfile, activeVehicleId, refreshKey, onExtraNav, userRole }) {
  ...
  // Inline trips block for company role
  const [showInlineTrips, setShowInlineTrips] = useState(false)
  const [allTrips, setAllTrips] = useState([])
  const [inlineTripsFilter, setInlineTripsFilter] = useState('all')
  const [inlineTripsPeriod, setInlineTripsPeriod] = useState('month')
  const [inlineTripsCustomFrom, setInlineTripsCustomFrom] = useState('')
  const [inlineTripsCustomTo, setInlineTripsCustomTo] = useState('')
  const [inlineTripsExpanded, setInlineTripsExpanded] = useState(null)
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Store all trips for inline trips block (company)
      if (profile?.role === 'company') setAllTrips(trips)
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {showInlineTrips && (() => {
                  const now2 = new Date()
                  let periodFrom, periodTo = now2
                  if (inlineTripsPeriod === 'week') {
                    periodFrom = new Date(now2)
                    periodFrom.setDate(periodFrom.getDate() - 7)
                  } else if (inlineTripsPeriod === 'custom' && inlineTripsCustomFrom) {
                    periodFrom = new Date(inlineTripsCustomFrom)
                    periodTo = inlineTripsCustomTo ? new Date(inlineTripsCustomTo) : now2
                  } else {
                    periodFrom = new Date(now2.getFullYear(), now2.getMonth(), 1)
                  }
                  let filtered = allTrips.filter(tr => {
                    const d = new Date(tr.created_at)
                    return d >= periodFrom && d <= periodTo
                  })
                  if (inlineTripsFilter !== 'all') {
                    filtered = filtered.filter(tr => tr.vehicle_id === inlineTripsFilter)
                  }
                  const fleetVehicles = fleetData?.vehicles || []
                  const tripsByVeh = {}
                  fleetVehicles.forEach(v => { tripsByVeh[v.id] = [] })
                  filtered.forEach(tr => {
                    if (tr.vehicle_id && tripsByVeh[tr.vehicle_id]) {
                      tripsByVeh[tr.vehicle_id].push(tr)
                    }
                  })
                  const periodBtnSt = (key) => ({
                    flex: 1,
                    padding: '8px 4px',
                    borderRadius: '8px',
                    border: inlineTripsPeriod === key ? '2px solid #f59e0b' : '1px solid ' + theme.border,
                    background: inlineTripsPeriod === key ? '#f59e0b22' : 'transparent',
                    color: inlineTripsPeriod === key ? '#f59e0b' : theme.dim,
                    fontSize: '13px',
                    fontWeight: 600,
                    cursor: 'pointer',
                    textAlign: 'center',
                  ...
                  return (
                    <div style={{ ...cardStyle, marginBottom: '12px' }}>
                      ...
                      {fleetVehicles.length > 0 && (
                        <select
                          value={inlineTripsFilter}
                          onChange={e => setInlineTripsFilter(e.target.value)}
                          style={{
                            width: '100%',
                            minHeight: '44px',
                            padding: '10px 12px',
                      ...
                      <div style={{ display: 'flex', gap: '6px', marginBottom: '10px' }}>
                        <button onClick={() => setInlineTripsPeriod('week')} style={periodBtnSt('week')}>
                          {t('trips.week')}
                        </button>
                        <button onClick={() => setInlineTripsPeriod('month')} style={periodBtnSt('month')}>
                          {t('common.month')}
                        </button>
                        <button onClick={() => setInlineTripsPeriod('custom')} style={periodBtnSt('custom')}>
                          {t('trips.periodFilter')}
                        </button>
                      </div>
                      {inlineTripsPeriod === 'custom' && (
                        <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
                          <input
                            type="date"
                            value={inlineTripsCustomFrom}
                            onChange={e => setInlineTripsCustomFrom(e.target.value)}
                            style={{
                          ...
                          <input
                            type="date"
                            value={inlineTripsCustomTo}
                            onChange={e => setInlineTripsCustomTo(e.target.value)}
                            style={{
                              flex: 1, padding: '8px 10px', borderRadius: '8px',
                              border: '1px solid ' + theme.border, background: theme.card, color: theme.text, fontSize: '13px',
                            }}
                      ...
                      {inlineTripsFilter !== 'all' ? (
                        filtered.length === 0 ? (
                          ...
                          filtered.map(tr => (
                            <div key={tr.id} style={{ padding: '8px 0', borderBottom: '1px solid ' + theme.border }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div>
                                  ...
                                  <div style={{ color: theme.dim, fontSize: '12px', marginTop: '2px' }}>
                                    {fmtDate(tr.created_at)} {'\u00b7'} {fmtNum(tr.distance_km || 0)} {unitSys === 'imperial' ? 'mi' : '\u043a\u043c'}
                                  </div>
                                </div>
                                <div style={{ color: '#22c55e', fontSize: '14px', fontWeight: 700, fontFamily: 'monospace', flexShrink: 0 }}>
                                  +{fmtNum(tr.income || 0)} {cs}
                                </div>
                              </div>
                            </div>
                          ))
                        ...
                        fleetVehicles.map(v => {
                          ...
                          const isExp = inlineTripsExpanded === v.id
                          const vLabel = ((v.brand || '') + ' ' + (v.model || '')).trim()
                          return (
                            <div key={v.id} style={{ borderBottom: '1px solid ' + theme.border, paddingBottom: '8px', marginBottom: '8px' }}>
                              <div
                                onClick={() => setInlineTripsExpanded(isExp ? null : v.id)}
                              ...
                              {isExp && (
                                vTrips.length === 0 ? (
                                  ...
                                  vTrips.map(tr => (
                                    <div key={tr.id} style={{ padding: '6px 0 6px 16px', borderTop: '1px solid ' + theme.border }}>
                                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <div>
